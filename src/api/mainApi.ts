...
export default class MainApi extends JsonApi {
  ...
  async getNumberOfStudentsOfTutor(tutorId: number): Promise<Response> {
    const { data: { result, errorMessage } } = await this.ApiRef.get(`/users/${tutorId}/number-of-students-of-tutor`)
    return {
      data: { result },
      errorMessage
    }
  ...
  ///////////// SETTING
  async getNumberOfStudentsPerTutor(): Promise<Response> {
    const { data: { result, errorMessage } } = await this.ApiRef.get('/settings/number-of-students-per-tutor')
    return {
      data: { result },
      errorMessage
    }
